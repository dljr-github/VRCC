"""Tests for ``MuteSync`` MuteChanged transitions, generation-guarded publish,
and stop/restart state handling, driven with a fake OSCQuery server. Split out
of test_mutesync.py (which covers should_caption modes and start gating) to
