...
def test_panel_is_centred_on_its_screen(qapp):
    ...
    Checked against the computed centre point, not containment: offscreen's
    geometry is large enough that an uncentred panel would still land
    somewhere inside it. A one-pixel tolerance absorbs integer rounding on
    an odd-sized available geometry or panel."""
    panel = BootPanel()
    try:
        panel.show()
        screen = panel.screen()
        ...
        actual = panel.frameGeometry().center()
