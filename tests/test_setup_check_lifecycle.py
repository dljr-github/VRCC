...
# -- reopening from Settings: gui.setup_check_done cleared, panel closed --


def test_recompute_reshows_the_panel_when_the_flag_is_cleared(qapp, tmp_path):
    """settings_simple.py's "bring back the setup steps" button only clears
    and persists gui.setup_check_done; it never touches this controller or
    the panel (test_setup_settings.py's own design constraint). The poll is
    the only thing that can notice the change and act on it."""
    store = _store(tmp_path)
