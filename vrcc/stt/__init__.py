"""Speech-to-text engines (faster-whisper, onnx-asr and SenseVoice)."""
...
def create_stt_engine(
    ...
    as onnx-asr-backed (Parakeet), a
    :class:`~vrcc.stt.sensevoice.SenseVoiceEngine` for the SenseVoice backend,
    else a :class:`~vrcc.stt.engine.SttEngine` (faster-whisper -- also the
    fallback for ids the registry doesn't know, preserving the old
    free-form-model-id behavior). All three share the same duck-typed
    load/warm_up/unload/transcribe contract. Engine modules import lazily so
    building the stack never pulls native runtimes early.
    """
    ...
    if spec is not None and spec.backend == "sensevoice":
        from vrcc.stt.sensevoice import SenseVoiceEngine

        return SenseVoiceEngine(cfg, spec, model_dir, bus)
