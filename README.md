...
Grab `VRCC-windows-x64.zip` from the
[latest release](https://github.com/dljr-github/VRCC/releases/latest), unzip
it anywhere, and run `VRCC.exe`. No Python setup needed — the first-run
wizard downloads the models for you.

The packaged exe runs on CPU: captions are identical to GPU, just a moment
slower, and the default models are sized to keep up. Installing from source
(below) is only for developers who want to contribute — or for NVIDIA GPU
owners who want the lowest possible latency via the `[cuda]` extra.
...
- **"No GPU detected" / everything runs on CPU** — the packaged exe is
  CPU-only; for GPU support install from source with the CUDA extra
  (`pip install -e .[cuda]`) and make sure your NVIDIA driver is ≥ 570.
